...
async function getCampaigns(accountId, dateFrom, dateTo) {
  ...
  return metaGet(`${id}/campaigns`, {
    fields: [
      ...
      "status",
      "objective",
      `insights.time_range(${timeRange}){spend,impressions,clicks,ctr,cpc,cpm,frequency,reach,inline_link_clicks,inline_link_click_ctr,cost_per_inline_link_click,actions,cost_per_action_type,purchase_roas}`,
    ].join(","),
    limit: 200,
  });
}
...
async function getAdsets(accountId, dateFrom, dateTo) {
  ...
  return metaGet(`${id}/adsets`, {
    fields: [
      ...
      "status",
      "campaign{name}",
      `insights.time_range(${timeRange}){spend,impressions,clicks,ctr,cpc,cpm,frequency,reach,inline_link_clicks,inline_link_click_ctr,cost_per_inline_link_click,actions,cost_per_action_type}`,
    ].join(","),
    limit: 200,
  });
}
...
async function getInsights(accountId, dateFrom, dateTo, breakdown, campaignIds) {
  ...
  const params = {
    fields: "spend,impressions,clicks,ctr,cpc,cpm,frequency,reach,inline_link_clicks,inline_link_click_ctr,cost_per_inline_link_click,actions,cost_per_action_type,purchase_roas",
...
async function getHourlyInsights(accountId, dateFrom, dateTo, campaignIds) {
  ...
  const params = {
    fields: "spend,impressions,clicks,frequency,reach,inline_link_clicks,inline_link_click_ctr,cost_per_inline_link_click,actions,cost_per_action_type",
    time_range: JSON.stringify({ since: dates.from, until: dates.to }),
